  ...
  bool initTexture(Map<String, dynamic> textureProperties, Texture texture) {
    ...
    if (textureCacheKey != textureProperties["__cacheKey"]) {
      ...
      if (webglTextures[textureCacheKey] == null) {
        ...
        if (texture.isOpenGLTexture) {
          final glTexture = texture as OpenGLTexture;
          webglTextures[textureCacheKey] = {
            "texture": glTexture.openGLTexture,
            "usedTimes": 0
          };
        } else {
          webglTextures[textureCacheKey] = {
        ...
        if (allocateMemory) {
          if (useTexStorage) {
            ...
            for (var i = 0; i < levels; i++) {
              state.texImage2D(_gl.TEXTURE_2D, i, glInternalFormat, width,
        ...
        if (mipmaps.isNotEmpty && supportsMips) {
          ...
          for (var i = 0, il = mipmaps.length; i < il; i++) {
            ...
            if (useTexStorage) {
              ...
              state.texImage2DIf(_gl.TEXTURE_2D, i, glInternalFormat, glFormat,
                  glType, mipmap);
            }
          }
      ...
      if (isCompressed) {
        ...
        for (var i = 0; i < 6; i++) {
          if (isDataTexture) {
            ...
            if (useTexStorage) {
              state.texSubImage2DIf(_gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0,
                  0, glFormat, glType, cubeImage[i]);
            } else {
              state.texImage2DIf(_gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0,
            ...
            for (var j = 0; j < mipmaps.length; j++) {
              ...
              if (useTexStorage) {
                state.texSubImage2DIf(_gl.TEXTURE_CUBE_MAP_POSITIVE_X + i,
                    j + 1, 0, 0, glFormat, glType, mipmap.image[i]);
              } else {
                state.texImage2DIf(_gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, j + 1,
