...
Vector3 getWorldPosition(Object3D object, Vector3 target) {
...
/// @return {number}
double getWorldDistance(Object3D obj1, Object3D obj2) {
  final a = getWorldPosition(obj1, t1);
  final b = getWorldPosition(obj2, t2);
...
/// @param {THREE.Vector3} target
Vector3 getCentroid(List<Vector3> positions, Vector3 target) {
  target.set(0, 0, 0);
...
setQuaternionFromDirection(Vector3 direction, Vector3 up, Quaternion target) {
  final x = t1;
  final y = t2;
  final z = t3;
  final m = m1;
  final el = m1.elements;
...
/// @param {THREE.Vector3} target
transformPoint(Vector3 vector, Matrix4 matrix, Vector3 target) {
  final e = matrix.elements;
  final x = (vector.x * e[0]) + (vector.y * e[4]) + (vector.z * e[8]) + e[12];
  final y = (vector.x * e[1]) + (vector.y * e[5]) + (vector.z * e[9]) + e[13];
  final z = (vector.x * e[2]) + (vector.y * e[6]) + (vector.z * e[10]) + e[14];
  final w = (vector.x * e[3]) + (vector.y * e[7]) + (vector.z * e[11]) + e[15];
